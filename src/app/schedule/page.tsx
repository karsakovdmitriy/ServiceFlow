            ...
            {weekDays.map((day, i) => (
              <div key={i} className="flex items-center justify-between py-3 border-b border-border-light last:border-none">
                <div>
                  ...
                  <div className="text-[12px] text-t3 mt-[1px]">{day.time}</div>
                </div>
                <div className={`w-10 h-[22px] rounded-full cursor-pointer transition-all relative shrink-0 ${day.on ? 'bg-green-custom' : 'bg-border-custom'}`}>
        ...
        <div>
          ...
          <div className="card">
            <div className="flex flex-col gap-2">
              {[
                ...
                <div key={i} className="flex items-center gap-[9px] p-[10px_12px] bg-bg-custom rounded-r-sm border border-border-light">
                  ...
                  <IconX size={15} className="text-t3 cursor-pointer hover:text-red-custom transition-all" />
                </div>
              ))}
            </div>
            <div className="flex gap-2 mt-3">
              ...
              />
              <button className="bg-surface border border-border-custom text-t1 text-[13px] font-medium p-[9px_14px] rounded-r-sm cursor-pointer whitespace-nowrap transition-all hover:bg-bg-custom hover:border-t3">+ Добавить</button>
            </div>
          </div>
        </div>
      </div>
